...
class MultiTimeframeConfluencePerpStrategy(BasePerpStrategy):
    """
    Multi-Timeframe Confluence Strategy for Crypto Sideways Trading
        ...
        try:
            ...
            if buy_confluence >= self.min_confluence and buy_confluence > sell_confluence:
                ...
                await self._condition_logger.log_condition(
                    ...
                    sell_confluence,
                    f"Sell signal with {sell_confluence} confluences: {', '.join(signal_details)}",
                    True,
                    context={'confluences': sell_confluence, 'details': signal_details}
                )
                confidence = min(sell_confluence / 5.0, 1.0)
                strength = min(sell_confluence / 5.0, 1.0)
                return self._perp_gate_signal('short', confidence, strength)
            else:
                await self._condition_logger.log_condition(
    ...
    async def analyze_pair(self, pair, min_candles=None, exchange_name: Optional[str] = None):
        ...
        for timeframe in timeframes:
            ...
            for name, value in indicators.items():
                ...
                if all(checks):
                    self.logger.info(f"[MultiTimeframeConfluence] Signal for {pair} on {timeframe}: LONG (all conditions met)")
                    results[timeframe] = ('long', 1, indicators)
                elif indicators['adx'] > self.adx_threshold and indicators['rsi'] < 50:
                    self.logger.info(f"[MultiTimeframeConfluence] Signal for {pair} on {timeframe}: SHORT (downside confluence met)")
                    results[timeframe] = ('short', 1, indicators)
                else:
