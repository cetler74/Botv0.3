...
class EngulfingMultiTfPerpStrategy(BasePerpStrategy):
    ...
    async def generate_signal(self, market_data, indicators_cache: Optional[dict] = None, pair: Optional[str] = None, timeframe: Optional[str] = None, exchange_adapter=None) -> Tuple[str, float, float]:
        ...
        try:
            ...
            pattern = self.detect_engulfing_pattern(ohlcv)
            if pattern not in {'bullish', 'bearish'}:
                return 'hold', 0.0, 0.0
            is_short_entry = pattern == 'bearish'
            if is_short_entry and not getattr(self, "allow_short", True):
                return 'hold', 0.0, 0.0
            if not is_short_entry and not getattr(self, "allow_long", True):
                return 'hold', 0.0, 0.0
            ...
            if ohlcv_macro is not None and not ohlcv_macro.empty and len(ohlcv_macro) >= 50:
                ...
                if self.reversal_mode:
                    ...
                    veto_macro = (
                        macro_trend_bearish
                        if is_short_entry
                        else macro_trend_bullish or macro_exhausted
                    )
                else:
                    veto_macro = macro_trend_bearish if is_short_entry else macro_trend_bullish
            else:
            ...
            if not hard_vetos:
                self.logger.info(
                    ...
                    f"macro_bullish={macro_trend_bullish} macro_bearish={macro_trend_bearish} "
                    f"macro_exhausted={macro_exhausted})"
                )
                return 'hold', 0.0, 0.0
            ...
            if self.enable_real_multi_tf and isinstance(tf_data, dict) and len(tf_data) > 1:
                try:
                    ...
                    if is_short_entry:
                        weights = self.timeframe_weights or {}
                        total_w = sum(float(weights.get(tf, 0.0)) for tf in self.target_timeframes)
                        mtf_bear = sum(1 for v in mtf_per_tf.values() if v == 'bearish')
                        if total_w <= 0:
                            mtf_weighted = mtf_bear / (float(len(self.target_timeframes)) or 1.0)
                        else:
                            mtf_weighted = sum(
                                float(weights.get(tf, 0.0))
                                for tf, v in mtf_per_tf.items() if v == 'bearish'
                            ) / total_w
                    if (mtf_bear if is_short_entry else mtf_bull) >= int(self.confirmation_timeframes):
            ...
            # All hard vetos passed AND directional engulfing detected.
            ...
            confidence = min(
                ...
                0.60
                + (adx - 22.0) / 100.0
                + (0.10 if (macro_trend_bearish if is_short_entry else macro_trend_bullish) else 0.0)
                + mtf_boost,
            )
            ...
            try:
                ...
                self.state.indicators.update({
                    ...
                    'position_intent': 'short' if is_short_entry else 'long',
            ...
            return 'short' if is_short_entry else 'long', float(confidence), float(strength)
    ...
    async def check_entry_signal(self, pair: str, market_data: Dict[str, pd.DataFrame], current_price: float) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            if pair in market_data:
                ...
                if signal in ['long', 'short', 'buy', 'sell'] and confidence >= self.min_confidence:
                    return True, {
                        'signal': signal,
