...
from strategy.hyperliquid.base_perp_strategy import BasePerpStrategy, StrategyState

logger = logging.getLogger(__name__)

class HeikinAshiPerpStrategy(BasePerpStrategy):
    def __init__(self, config, exchange, database, redis_client=None, exchange_name=None):
        ...
        self.short_confidence = float(strat_cfg.get('short_confidence', strat_cfg.get('min_confidence_score', 0.55)))
        self.short_strength = float(strat_cfg.get('short_strength', self.short_confidence))
    ...
    async def analyze_pair(self, pair, min_candles=None, exchange_name=None, market_data=None):
        ...
        for i, timeframe in enumerate(timeframes):
            ...
            for name in core_indicators:
                ...
                if signal_strength >= tf_signal_threshold:
                    ...
                    timeframe_signals[timeframe] = ('long', signal_strength, indicators)
                else:
                    failed_checks = []
        ...
        if confluence_score >= self.min_confluence_score:
            ...
            macro_buy  = macro_sig[0] in ('buy', 'long')
            signal_buy = signal_sig[0] in ('buy', 'long')
            exec_buy   = exec_sig[0] in ('buy', 'long')
    ...
    async def generate_signal(self, market_data, *args, **kwargs):
        ...
        if signal not in ("long", "short") and self.allow_short:
            short_signal = await self._generate_short_signal(market_data, pair)
            if short_signal[0] == "short":
                return short_signal

        return self._perp_gate_signal(signal, confidence, strength)

    def _resolve_short_df(self, market_data, key: str, fallbacks: Tuple[str, ...]) -> Optional[pd.DataFrame]:
        if isinstance(market_data, dict):
            for tf in (key, *fallbacks):
                df = market_data.get(tf)
                if isinstance(df, pd.DataFrame) and not df.empty:
                    return df.sort_index()
            return None
        return market_data.sort_index() if isinstance(market_data, pd.DataFrame) else None

    async def _generate_short_signal(self, market_data, pair: Optional[str]) -> Tuple[str, float, float]:
        """Mirrored HA/MACD/EMA short entry for Hyperliquid perps."""
        try:
            exec_df = self._resolve_short_df(market_data, self.execution_timeframe, ("15m", "1h"))
            trend_df = self._resolve_short_df(market_data, self.signal_timeframe, ("1h", "15m"))
            macro_df = self._resolve_short_df(market_data, self.macro_timeframe, ("4h", "1h"))
            if exec_df is None or trend_df is None:
                return "hold", 0.0, 0.0
            min_len = max(self.ema_slow_period, self.rsi_period, self.macd_slow + self.macd_signal, 50)
            required = {"open", "high", "low", "close", "volume"}
            if len(exec_df) < min_len or len(trend_df) < min_len:
                return "hold", 0.0, 0.0
            if not required.issubset(exec_df.columns) or not required.issubset(trend_df.columns):
                return "hold", 0.0, 0.0

            exec_close = exec_df["close"].astype(float)
            trend_close = trend_df["close"].astype(float)
            ema_fast = ta.ema(exec_close, length=self.ema_fast_period)
            ema_slow = ta.ema(exec_close, length=self.ema_slow_period)
            trend_ema_slow = ta.ema(trend_close, length=self.ema_slow_period)
            rsi = ta.rsi(exec_close, length=self.rsi_period)
            if (
                ema_fast is None
                or ema_slow is None
                or trend_ema_slow is None
                or rsi is None
                or pd.isna(ema_fast.iloc[-1])
                or pd.isna(ema_slow.iloc[-1])
                or pd.isna(trend_ema_slow.iloc[-1])
                or pd.isna(rsi.iloc[-1])
            ):
                return "hold", 0.0, 0.0

            price = float(exec_close.iloc[-1])
            exec_below_fast = price < float(ema_fast.iloc[-1])
            exec_below_slow = price < float(ema_slow.iloc[-1])
            trend_below_slow = float(trend_close.iloc[-1]) < float(trend_ema_slow.iloc[-1])
            macro_below_slow = True
            if macro_df is not None and len(macro_df) >= self.ema_slow_period + 5 and "close" in macro_df.columns:
                macro_close = macro_df["close"].astype(float)
                macro_ema = ta.ema(macro_close, length=self.ema_slow_period)
                if macro_ema is not None and not pd.isna(macro_ema.iloc[-1]):
                    macro_below_slow = float(macro_close.iloc[-1]) < float(macro_ema.iloc[-1])

            ha_pattern = self.detect_ha_pattern(self.compute_heikin_ashi(exec_df))
            macd_state = self.compute_macd_state(exec_df)
            rsi_now = float(rsi.iloc[-1])
            bearish_ha = bool(ha_pattern.get("bearish_continuation") or ha_pattern.get("bearish_reversal"))
            bearish_macd = bool(macd_state.get("bearish") or macd_state.get("bearish_cross"))
            rsi_ok = rsi_now <= float(getattr(self, "rsi_uptrend_max", 70))
            structural_down = exec_below_fast and exec_below_slow and trend_below_slow and macro_below_slow

            checks = [structural_down, bearish_ha, bearish_macd, rsi_ok]
            confidence = sum(1 for ok in checks if ok) / len(checks)
            if structural_down and bearish_macd and confidence >= self.short_confidence:
                self.state.last_signal = "short"
                self.state.indicators = {
                    "entry_reason": (
                        "heikin_ashi_short:"
                        f"confidence={confidence:.2f}|rsi={rsi_now:.1f}|"
                        f"ha_red_streak={ha_pattern.get('red_streak', 0)}|macd_bear"
                    ),
                    "short_structural_down": structural_down,
                    "short_bearish_ha": bearish_ha,
                    "short_bearish_macd": bearish_macd,
                    "short_rsi_ok": rsi_ok,
                    "ha_pattern": ha_pattern,
                    "macd_state": macd_state,
                }
                logger.info(
                    "[HeikinAshiStrategy] Final mirrored short signal for %s: SHORT confidence=%.2f",
                    pair or self.state.pair,
                    confidence,
                )
                return self._perp_gate_signal("short", confidence, max(self.short_strength, confidence))
            return "hold", 0.0, 0.0
        except Exception as exc:
            logger.debug("[HeikinAshiStrategy] mirrored short generation failed for %s: %s", pair, exc)
            return "hold", 0.0, 0.0
    ...
    def detect_ha_pattern(self, ha_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        Returns a dict with:
          ...
          bearish_continuation : bool — red_streak >= ha_min_green_streak AND
                                        the LAST `ha_min_green_streak` candles
                                        all show no upper wick
        ...
        if red_streak >= self.ha_min_green_streak:
            no_upper_wick_ok = True
            for j in range(self.ha_min_green_streak):
                idx = len(ha_df) - 1 - j
                ref_j = max(abs(ha_open.iloc[idx]), 1e-12)
                if (ha_high.iloc[idx] - ha_open.iloc[idx]) / ref_j > tol:
                    no_upper_wick_ok = False
                    break
            bearish_continuation = no_upper_wick_ok
        ...
        try:
            ...
            for timeframe, (signal, confidence, indicators) in timeframe_signals.items():
                if timeframe in weights:
                    ...
                    signal_score = confidence if signal in ('buy', 'long') else 0.0
    ...
    async def _should_exit_legacy(self) -> Tuple[bool, Optional[str]]:
