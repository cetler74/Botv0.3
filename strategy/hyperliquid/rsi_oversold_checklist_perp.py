...
class RsiOversoldChecklistPerpStrategy(BasePerpStrategy):
    ...
    async def generate_signal(
        ...
        try:
            ...
            overbought_level = float(
                getattr(self, "overbought_reclaim_level", 70.0)
                or getattr(self, "rsi_overbought_level", 70.0)
            )
            exec_rsi_short_ok = (100 - self.exec_rsi_max) <= exec_rsi_now <= (100 - self.exec_rsi_min)
            trend_rsi_short_ok = (trend_rsi_now < (100 - self.trend_rsi_min)) if self.require_multi_tf_rsi else True
            price_below_ema = price_now < ema20_now if self.require_price_above_ema20 else True
            macro_bear_ok = (ema50_now < ema200_now) if self.require_macro_trend_filter else True
            rsi_short_trigger = False
            if exec_rsi is not None and len(exec_rsi) >= self.reclaim_cross_bars + 1:
                prior = exec_rsi.iloc[-(self.reclaim_cross_bars + 1):-1]
                rsi_short_trigger = bool((prior >= overbought_level).any() and exec_rsi_now < overbought_level)
            checklist_sell = all(
                [
                    rsi_short_trigger,
                    exec_rsi_short_ok,
            ...
            ) and getattr(self, "allow_short", True)

            signal = "long" if checklist_buy else ("short" if checklist_sell else "hold")
            confidence = 0.0
            strength = 0.0
            if checklist_sell and not checklist_buy:
                confidence = float(getattr(self, "sell_confidence", self.buy_confidence))
                strength = float(getattr(self, "sell_strength", self.buy_strength))
            elif checklist_buy:
