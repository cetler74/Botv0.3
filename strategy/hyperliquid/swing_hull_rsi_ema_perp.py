"""
Swing Hull/RSI/EMA perp strategy (1h).

Port of TradingView "Swing Hull/rsi/EMA Strategy":
  - Long: Hull direction (n1 > n2) + price cross-down through fast EMA
  - Short: Hull direction (n1 < n2) + price cross-up through fast EMA
  - RSI structural exits are still handled by the orchestrator.
"""
...
class SwingHullRsiEmaPerpStrategy(BasePerpStrategy):
    """Perp swing entry on 1h; structural RSI exit via orchestrator."""
    ...
    def _evaluate_entry_on_df(
        ...
        """Return (buy_ok, sell_ok, indicator snapshot) on closed bar indices."""
        ...
        hull_bearish = float(n1.iloc[closed_idx]) < float(n2.iloc[closed_idx])
        ...
        ema_cross_up = c_closed > ef_closed and c_prev < ef_prev
        ...
        buy_ok = self.allow_long and hull_bullish and ema_cross_down
        sell_ok = self.allow_short and hull_bearish and ema_cross_up
        indicators = {
            "n1": float(n1.iloc[closed_idx]),
            "n2": float(n2.iloc[closed_idx]),
            "hull_bullish": hull_bullish,
            "hull_bearish": hull_bearish,
            "ema_fast": ef_closed,
            "ema_slow": es_closed,
            "rsi": rsi_closed,
        ...
        return buy_ok, sell_ok, indicators
    ...
    async def generate_signal(
        ...
        try:
            ...
            buy_ok, sell_ok, indicators = self._evaluate_entry_on_df(exec_df)
            hull_bullish = indicators.get("hull_bullish", False)
            hull_bearish = indicators.get("hull_bearish", False)
            ema_cross_down = indicators.get("ema_cross_down", False)
            ema_cross_up = indicators.get("ema_cross_up", False)
            ...
            if buy_ok:
                ...
                self.logger.info(
                    "[SwingHullRsiEma] %s LONG conf=%.2f str=%.2f hull_bullish=%s ema_cross_down=%s",
                    pair or self.state.pair,
                    self.min_confidence,
                    self.buy_strength,
                    hull_bullish,
            ...
            if sell_ok:
                ...
                self.logger.info(
                    "[SwingHullRsiEma] %s SHORT conf=%.2f str=%.2f hull_bearish=%s ema_cross_up=%s",
                    pair or self.state.pair,
                    self.min_confidence,
                    self.buy_strength,
                    hull_bearish,
            ...
            if self.allow_short and not hull_bearish:
                hold_reason.append("hull_not_bearish")
            if self.allow_short and not ema_cross_up:
                hold_reason.append("no_ema_cross_up")
            self.state.last_signal = "hold"
            self.logger.info(
