...
class SuperTrendPerpStrategy(BasePerpStrategy):
    ...
    def _bearish_trend_confirmed(self, st: pd.DataFrame, closed_idx: int) -> bool:
        """Require N consecutive closed bars with trend==-1 (includes flip bar)."""
        n = int(getattr(self, "confirm_bearish_bars", self.confirm_bullish_bars) or 1)
        if n <= 1:
            return int(st["trend"].iloc[closed_idx]) == -1
        start = closed_idx - n + 1
        if start < -len(st):
    ...
    async def generate_signal(
        ...
        try:
            ...
            if bullish_flip:
                ...
                self.logger.info(
                    "[SuperTrend] %s LONG conf=%.2f str=%.2f 15m_flip %s_trend=%s adx=%.1f vol_ok=%s",
                    pair or self.state.pair,
                    self.min_confidence,
                    self.buy_strength,
                    self.trend_timeframe,
            ...
            bearish_flip = trend_now == -1 and trend_prev == 1
            if bearish_flip and getattr(self, "allow_short", True):
                trend_1h_bear_ok = (trend_1h == -1) if trend_1h is not None else True
                if trend_1h_bear_ok and self._bearish_trend_confirmed(st, closed_idx):
                    if (not self.require_close_above_dn or c_closed < up_band) and adx_ok and vol_ok:
                        sell_conf = float(getattr(self, "sell_confidence", self.min_confidence))
                        sell_str = float(getattr(self, "sell_strength", self.buy_strength))
                        self.state.last_signal = "short"
                        return self._perp_gate_signal("short", sell_conf, sell_str)
