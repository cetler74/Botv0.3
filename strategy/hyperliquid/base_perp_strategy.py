"""
Hyperliquid perpetual fork — full TA port from spot (do not import spot strategy classes).
Venue: hyperliquid perps (long + short).
"""
...
@dataclass
class PerpStrategyState:
    """Current state of a trading strategy."""
    pair: str
    position: str  # 'long' | 'short' | 'none' (perp)
...
# Spot-fork modules import this name from base_perp_strategy.
StrategyState = PerpStrategyState


class BasePerpStrategy(ABC):
    ...
    def _perp_gate_signal(self, signal: str, confidence: float, strength: float):
    ...
    async def should_exit(
        ...
        **kwargs: Any,
        ...
        """
        Perp orchestrator calls with (position_side, entry_price, current_price).
        Spot-style ``should_exit()`` with no args is still used inside strategies.
        """
        if position_side is not None and entry_price is not None and current_price is not None:
            return await self.should_exit_perp(
                str(position_side).lower(),
                float(entry_price),
                float(current_price),
                market_data=market_data,
            )
        return await self._should_exit_legacy()

    async def should_exit_perp(
        self,
        position_side: str,
        entry_price: float,
        current_price: float,
        ...
        """Override in subclass for side-aware exits; default uses SL/TP %% from parameters."""
        pnl_pct = self._perp_pnl_pct(position_side, entry_price, current_price)
        sl = float(getattr(self, "stop_loss_pct", 0) or (self.config.get("parameters", {}) or {}).get("stop_loss_pct", 1.5))
        tp = float(getattr(self, "take_profit_pct", 0) or (self.config.get("parameters", {}) or {}).get("take_profit_pct", 2.5))
        if sl > 0 and pnl_pct <= -abs(sl):
            return True, f"{self.__class__.__name__}: stop_loss {pnl_pct:.2f}%"
        if tp > 0 and pnl_pct >= abs(tp):
            return True, f"{self.__class__.__name__}: take_profit {pnl_pct:.2f}%"
        legacy = await self._should_exit_legacy()
        if legacy is True:
            return True, f"{self.__class__.__name__}: legacy_exit"
        if isinstance(legacy, tuple):
            return legacy
        return False, ""

    def _perp_pnl_pct(self, position_side: str, entry: float, current: float) -> float:
    ...
    @abstractmethod
    async def _should_exit_legacy(self) -> Any:
        """Spot-style exit hook (subclasses implement former should_exit body)."""
        pass

    async def execute_trade(self, signal_type: str) -> bool:
        """Execute a trade based on the signal."""
        try:
            entry_intent = str(signal_type or "").lower().strip()
            if entry_intent == "buy":
                entry_intent = "long"
            elif entry_intent == "sell":
                entry_intent = "short"
            if entry_intent not in {"long", "short"}:
                logger.warning(f"Invalid Hyperliquid perp entry intent: {signal_type}")
                return False
            ...
            # Execute order
            order_side = "buy" if entry_intent == "long" else "sell"
            ...
            # Update state
            self.state.position = entry_intent
            ...
            logger.info(f"Executed {entry_intent} order for {self.state.pair}")
            return True
