"""
MACD + 200 EMA + session VWAP long-only scalper.
...
Emits buy/hold only; exits are orchestrator-managed.
"""
...
class MACDEMAVWAPScalperStrategy(BaseStrategy):
    """Triple-confirmation long scalper: trend (200 EMA), mean anchor (VWAP), momentum (MACD cross)."""
    ...
    def __init__(
        ...
        # Long-only scalper: optional hard skip (orchestrator bypasses consensus for this strategy).
    ...
    async def generate_signal(
        ...
        try:
            ...
            if current_regime in self.blocked_regimes:
                self.logger.info(
                    "[MACDEMAVWAPScalper] %s HOLD: regime=%s in blocked_regimes",
            ...
            if buy_ok:
                ...
                self.logger.info(
                    "[MACDEMAVWAPScalper] %s BUY conf=%.2f str=%.2f %s",
                    pair or self.state.pair,
                    self.min_confidence,
                    self.buy_strength,
                    reasons,
