...
async def select_top_pairs_ccxt(exchange_id, base_currency='USDC', num_pairs=10, market_type='spot'):
    ...
    try:
        ...
        if not exchange.markets:
            logger.error(f"[CCXT PairSelector] Failed to load markets for {exchange_id}")
        ...
        if exchange_id.lower() == 'cryptocom':
            ...
            # If validation failed or returned insufficient pairs, use predefined high-quality pairs
            if len(validated_pairs) < num_pairs:
                logger.warning(f"[PairSelector] Insufficient validated pairs ({len(validated_pairs)}), using predefined pairs for crypto.com")
                predefined_cryptocom_pairs = [
                    'CRO/USD', '1INCH/USD', 'AAVE/USD', 'ACA/USD', 'ACH/USD', 'ACT/USD', 'BTC/USD', 'ETH/USD',
                    'XRP/USD', 'DOT/USD', 'MATIC/USD', 'UNI/USD', 'AVAX/USD', 'LINK/USD', 'LTC/USD', 'ADA/USD',
                    'SOL/USD', 'ATOM/USD', 'ALGO/USD', 'NEAR/USD'
                ]
                # Combine validated pairs with predefined ones, avoiding duplicates
                combined_pairs = list(validated_pairs)
                for pair in predefined_cryptocom_pairs:
                    if pair not in combined_pairs and len(combined_pairs) < num_pairs:
                        combined_pairs.append(pair)
                validated_pairs = combined_pairs[:num_pairs]
                logger.info(f"[PairSelector] Using {len(validated_pairs)} combined pairs for crypto.com")
            
            sanitized_pairs = validated_pairs[:num_pairs]
            logger.info(f"[PairSelector] Final result: {len(validated_pairs)} out of {len(test_pairs)} crypto.com pairs")
        else:
        ...
        logger.error(f"[CCXT PairSelector] Error selecting pairs for {exchange_id}: {e}")
        try:
            await exchange.close()
        except:
            pass
...
async def _get_blacklisted_pairs():
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[PairSelector] Fallback config loading failed: {fallback_error}")
        return []
