...
"""
Fix All Exit Prices
Comprehensive fix for all closed trades using Redis data
"""
...
def main():
    """Main function"""
    logger.info("🚀 Starting comprehensive exit price fix...")
    ...
    for trade in trades:
        ...
        # Determine which price to use for exit_price
        # If filled_price equals entry_price, use order_price (limit order price)
        # Otherwise, use filled_price (actual execution price)
        if abs(filled_price - entry_price) < 0.000001:
            # Filled at entry price, use order price
            new_exit_price = order_price
            logger.info(f"🔍 Trade {trade_id} ({pair}): filled at entry price {filled_price}, using order price {order_price}")
        else:
            # Filled at different price, use filled price
            new_exit_price = filled_price
            logger.info(f"🔍 Trade {trade_id} ({pair}): filled at {filled_price}, using filled price")
        
        # Check if we need to fix
        if abs(current_exit_price - new_exit_price) < 0.000001:
            logger.debug(f"✅ Trade {trade_id} already correct")
            skipped_count += 1
            continue
        
        # Calculate new PnL
        new_realized_pnl = (new_exit_price - entry_price) * position_size - fees
        
        # Update trade
        update_sql = f"UPDATE trading.trades SET exit_price = {new_exit_price}, realized_pnl = {new_realized_pnl}, updated_at = NOW() WHERE trade_id = '{trade_id}';"
        
        update_result = run_sql_command(update_sql)
        if update_result.returncode == 0:
            logger.info(f"✅ Fixed trade {trade_id} ({pair}): {current_exit_price} → {new_exit_price}, PnL: {current_pnl:.4f} → {new_realized_pnl:.4f}")
            fixed_count += 1
        else:
    ...
    logger.info(f"🎉 Comprehensive exit price fix completed!")
