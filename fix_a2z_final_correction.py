#!/usr/bin/env python3
"""
Final A2Z position correction - update all trades to match current exchange balance
"""
...
async def fix_a2z_final_correction():
    """Fix A2Z position size to match current exchange balance"""
    
    async with httpx.AsyncClient() as client:
        try:
            print("🔍 FINAL A2Z POSITION CORRECTION")
            print("=" * 60)
            ...
            if abs(discrepancy) > 0.01:
                ...
                if actual_a2z_balance > 0:
                    # Strategy: Keep only 1 trade with the correct balance
                    print(f"🔄 Consolidating {len(a2z_trades)} trades into 1 with {actual_a2z_balance:,} A2Z")
                    ...
                    # No balance on exchange - close all trades
                    print(f"❌ No A2Z balance on exchange - closing all {len(a2z_trades)} trades")
                    ...
                    print(f"\n🎯 RESULT:")
                    print(f"   ✅ Closed {closed_count} trades (no exchange balance)")
                    print(f"   📊 Database now matches exchange reality")
            else:
                print("✅ Position sizes are already accurate!")
            ...
            if final_response.status_code == 200:
                ...
                elif len(final_a2z_trades) == 0:
                    print(f"✅ No A2Z trades (matches exchange reality)")
                else:
...
if __name__ == "__main__":
    asyncio.run(fix_a2z_final_correction())
