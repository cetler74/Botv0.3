#!/usr/bin/env python3
"""
Analyze Real Trades - Analyze the actual trades from the correct database
"""
...
def get_db_connection(database_name="trading_bot"):
...
def main():
    """Main function"""
    print(f"\n📊 ANALYZING REAL TRADES FROM CORRECT DATABASE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 100)
    
    # Try different databases
    databases_to_try = ["trading_bot", "multi_trading_bot", "trading_bot_margin"]
    ...
    for db_name in databases_to_try:
        ...
        try:
            ...
            schema_query = "SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'trading'"
            schemas = execute_query(connection, schema_query)
            
            if not schemas:
                print(f"   ❌ No 'trading' schema in {db_name}")
                connection.close()
                continue
            
            # Check if trades table exists
            table_query = "SELECT table_name FROM information_schema.tables WHERE table_schema = 'trading' AND table_name = 'trades'"
            tables = execute_query(connection, table_query)
            
            if not tables:
                print(f"   ❌ No 'trades' table in {db_name}")
                connection.close()
                continue
            ...
            if count_result and count_result[0]['total_trades'] > 0:
                print(f"   ✅ Found {count_result[0]['total_trades']} trades in {db_name}")
                ...
                if trades:
                    print(f"\n📊 Found {len(trades)} trades in {db_name}")
                    print()
                    ...
                    print(f"   ❌ No trades found in {db_name}")
            else:
                print(f"   ❌ No trades in {db_name}")
