-- OMS Phase 1: Event Sourcing Schema Migration
...
CREATE TABLE IF NOT EXISTS trading.order_mappings (
    ...
    trade_id UUID REFERENCES trading.trades(trade_id) -- Link to trade record
);
...
CREATE OR REPLACE VIEW trading.latest_events_per_aggregate AS
...
CREATE OR REPLACE VIEW trading.order_status_summary AS
SELECT 
    om.local_order_id,
    om.exchange,
    om.symbol,
...
VALUES ('oms', 'phase1_migration', 'completed', 'system')
